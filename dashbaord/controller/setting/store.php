...
class Store extends \Opencart\System\Engine\Controller {
	...
	public function getList(): string {
		...
		foreach ($results as $result) {
			if ($result['store_id'] == 0) {
				// Add edit link for default store
				$result['edit'] = $this->url->link('setting/setting', 'user_token=' . $this->session->data['user_token']);
			} else {
				$result['edit'] = $this->url->link('setting/store.form', 'user_token=' . $this->session->data['user_token'] . '&store_id=' . $result['store_id']);
			}
			$data['stores'][] = $result;
		}

		$store_total = $this->model_setting_store->getTotalStores() + 1; // +1 for default store
	...
	public function save(): void {
		...
		if (!isset($this->request->post['config_country_id']) || !$this->request->post['config_country_id']) {
			...
			if ($zone_total && (!isset($this->request->post['config_zone_id']) || !$this->request->post['config_zone_id'])) {
